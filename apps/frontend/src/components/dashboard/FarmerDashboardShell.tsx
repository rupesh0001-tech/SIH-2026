...
function DashboardPage({ user, metrics, currentBookings, onCreateBooking, onSelectBookingForQR, onBookMandi, onNavigate }: SharedProps) {
  return <PageFrame><WelcomeBanner user={user} onCreateBooking={onCreateBooking} /><FarmerMetricsRow {...metrics} /><section className="space-y-4"><SectionHeading title="Active Unloading Bookings" route="/bookings" onNavigate={onNavigate} /><BookingsTableSection activeTab="current" onTabChange={() => undefined} displayedList={currentBookings.slice(0, 2)} currentBookingsCount={currentBookings.length} previousBookingsCount={0} onOpenCreateModal={onCreateBooking} onSelectBookingForQR={onSelectBookingForQR} /></section><section className="space-y-4"><SectionHeading title="Suggested Mandis with Open Arrival Slots" route="/find-mandi" onNavigate={onNavigate} /><SuggestedMandisSection mandis={SUGGESTED_MANDIS.slice(0, 2)} onBookMandi={onBookMandi} /></section><section className="space-y-4"><SectionHeading title="Latest Alerts" route="/bookings" onNavigate={onNavigate} /><RecentMessagesSection messages={RECENT_MESSAGES.slice(0, 2)} /></section></PageFrame>;
}
